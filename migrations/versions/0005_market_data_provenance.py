...
def upgrade() -> None:
    with op.batch_alter_table('candles', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('ingested_at', app.db.types.UTCDateTime(timezone=True), nullable=True))
    ...
    with op.batch_alter_table('virtual_portfolios', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('session_start_equity', app.db.types.Money(precision=18, scale=6), nullable=True))

    with op.batch_alter_table('virtual_positions', schema=None) as batch_op:
        batch_op.add_column(sa.Column('corporate_actions_applied_through', app.db.types.UTCDateTime(timezone=True), nullable=True))
