...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "drawdown <= 0", name=op.f("ck_portfolio_snapshots_drawdown_non_positive")
        ),
        sa.ForeignKeyConstraint(
            ["simulation_profile_id"],
            ["simulation_profiles.id"],
            name=op.f("fk_portfolio_snapshots_simulation_profile_id_simulation_profiles"),
            ondelete="CASCADE",
        ),
    ...
    with op.batch_alter_table("portfolio_snapshots", schema=None) as batch_op:
        batch_op.create_index(
            "ix_portfolio_snapshots_profile_timestamp",
            ["simulation_profile_id", "timestamp"],
            unique=False,
        )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "initial_capital > 0", name=op.f("ck_virtual_portfolios_initial_capital_positive")
        ),
        sa.CheckConstraint(
            "max_drawdown <= 0", name=op.f("ck_virtual_portfolios_max_drawdown_non_positive")
        ),
        sa.CheckConstraint(
            "peak_equity >= 0", name=op.f("ck_virtual_portfolios_peak_equity_non_negative")
        ),
        sa.CheckConstraint(
            "total_fees >= 0", name=op.f("ck_virtual_portfolios_total_fees_non_negative")
        ),
        sa.CheckConstraint(
            "trade_count >= 0", name=op.f("ck_virtual_portfolios_trade_count_non_negative")
        ),
        sa.ForeignKeyConstraint(
            ["simulation_profile_id"],
            ["simulation_profiles.id"],
            name=op.f("fk_virtual_portfolios_simulation_profile_id_simulation_profiles"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_virtual_portfolios")),
        sa.UniqueConstraint(
            "simulation_profile_id", name=op.f("uq_virtual_portfolios_simulation_profile_id")
        ),
    )
    op.create_table(
        "decision_outcomes",
        ...
        sa.CheckConstraint(
            "bars_evaluated >= 1", name=op.f("ck_decision_outcomes_bars_evaluated_positive")
        ),
        sa.ForeignKeyConstraint(
            ["instrument_id"],
            ["instruments.id"],
            name=op.f("fk_decision_outcomes_instrument_id_instruments"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["trade_decision_id"],
            ["trade_decisions.id"],
            name=op.f("fk_decision_outcomes_trade_decision_id_trade_decisions"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_decision_outcomes")),
        sa.UniqueConstraint(
            "trade_decision_id", name=op.f("uq_decision_outcomes_trade_decision_id")
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("OPEN", "CLOSED", name="positionstatus", native_enum=False, length=8),
            nullable=False,
        ),
        ...
        sa.Column(
            "unrealized_pnl",
            app.db.types.Money(precision=18, scale=6),
            server_default="0",
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            "status <> 'CLOSED' OR (exit_price IS NOT NULL AND exit_timestamp IS NOT NULL AND exit_reason IS NOT NULL)",
            name=op.f("ck_virtual_positions_closed_requires_exit"),
        ),
        sa.CheckConstraint(
            "average_entry_price > 0", name=op.f("ck_virtual_positions_entry_price_positive")
        ),
        sa.CheckConstraint("quantity > 0", name=op.f("ck_virtual_positions_quantity_positive")),
        sa.CheckConstraint(
            "stop_loss IS NULL OR stop_loss > 0",
            name=op.f("ck_virtual_positions_stop_loss_positive"),
        ),
        sa.ForeignKeyConstraint(
            ["instrument_id"],
            ["instruments.id"],
            name=op.f("fk_virtual_positions_instrument_id_instruments"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["originating_signal_id"],
            ["signals.id"],
            name=op.f("fk_virtual_positions_originating_signal_id_signals"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["originating_trade_decision_id"],
            ["trade_decisions.id"],
            name=op.f("fk_virtual_positions_originating_trade_decision_id_trade_decisions"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["simulation_profile_id"],
            ["simulation_profiles.id"],
            name=op.f("fk_virtual_positions_simulation_profile_id_simulation_profiles"),
            ondelete="CASCADE",
        ),
    ...
    with op.batch_alter_table("virtual_positions", schema=None) as batch_op:
        batch_op.create_index(
            "ix_virtual_positions_instrument_status", ["instrument_id", "status"], unique=False
        )
        batch_op.create_index(
            "ix_virtual_positions_profile_status", ["simulation_profile_id", "status"], unique=False
        )
        batch_op.create_index(
            "uq_virtual_positions_open_per_instrument",
            ["simulation_profile_id", "instrument_id"],
            unique=True,
    ...
    op.create_table(
        ...
        sa.Column(
            "order_type",
            sa.Enum("MARKET", "LIMIT", name="ordertype", native_enum=False, length=8),
            nullable=False,
        ),
        sa.Column(
            "status",
        ...
        sa.Column(
            "fees", app.db.types.Money(precision=18, scale=6), server_default="0", nullable=False
        ),
        sa.Column(
            "spread_cost",
            app.db.types.Money(precision=18, scale=6),
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_virtual_orders_filled_requires_price"),
        ),
        sa.CheckConstraint(
            "status <> 'REJECTED' OR rejection_reason IS NOT NULL",
            name=op.f("ck_virtual_orders_rejected_requires_reason"),
        ),
        ...
        sa.ForeignKeyConstraint(
            ["instrument_id"],
            ["instruments.id"],
            name=op.f("fk_virtual_orders_instrument_id_instruments"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["position_id"],
            ["virtual_positions.id"],
            name=op.f("fk_virtual_orders_position_id_virtual_positions"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["simulation_profile_id"],
            ["simulation_profiles.id"],
            name=op.f("fk_virtual_orders_simulation_profile_id_simulation_profiles"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["trade_decision_id"],
            ["trade_decisions.id"],
            name=op.f("fk_virtual_orders_trade_decision_id_trade_decisions"),
            ondelete="SET NULL",
        ),
    ...
    with op.batch_alter_table("virtual_orders", schema=None) as batch_op:
        ...
        batch_op.create_index(
            "ix_virtual_orders_profile_requested_at",
            ["simulation_profile_id", "requested_at"],
            unique=False,
        )
    ...
    op.create_table(
        ...
        sa.Column(
            "max_favorable_excursion", app.db.types.Money(precision=18, scale=6), nullable=True
        ),
        sa.Column(
            "max_adverse_excursion", app.db.types.Money(precision=18, scale=6), nullable=True
        ),
        sa.Column(
            "outcome",
            sa.Enum("WIN", "LOSS", "BREAKEVEN", name="tradeoutcome", native_enum=False, length=12),
            nullable=False,
        ),
        sa.Column(
            "created_at",
        ...
        sa.ForeignKeyConstraint(
            ["instrument_id"],
            ["instruments.id"],
            name=op.f("fk_virtual_trades_instrument_id_instruments"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["originating_signal_id"],
            ["signals.id"],
            name=op.f("fk_virtual_trades_originating_signal_id_signals"),
            ondelete="SET NULL",
        ),
        sa.ForeignKeyConstraint(
            ["position_id"],
            ["virtual_positions.id"],
            name=op.f("fk_virtual_trades_position_id_virtual_positions"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["simulation_profile_id"],
            ["simulation_profiles.id"],
            name=op.f("fk_virtual_trades_simulation_profile_id_simulation_profiles"),
            ondelete="CASCADE",
        ),
    ...
    with op.batch_alter_table("virtual_trades", schema=None) as batch_op:
        ...
        batch_op.create_index(
            "ix_virtual_trades_profile_exit",
            ["simulation_profile_id", "exit_timestamp"],
            unique=False,
        )
    ...
    with op.batch_alter_table("risk_profiles", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column(
                "stop_loss_atr_multiple", app.db.types.Money(precision=9, scale=6), nullable=True
            )
        )
        batch_op.add_column(
            sa.Column(
                "take_profit_r_multiple", app.db.types.Money(precision=9, scale=6), nullable=True
            )
        )
        batch_op.add_column(sa.Column("max_holding_bars", sa.Integer(), nullable=True))
        batch_op.add_column(
            sa.Column("require_stop_loss", sa.Boolean(), server_default="true", nullable=False)
        )
        batch_op.add_column(
            sa.Column("allow_pyramiding", sa.Boolean(), server_default="false", nullable=False)
        )
        batch_op.add_column(
            sa.Column("max_quote_age_seconds", sa.Integer(), server_default="900", nullable=False)
        )
