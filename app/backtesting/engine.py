...
from app.scanner.timeframes import PRIMARY_TIMEFRAME
...
"""Symbols processed per transaction. Bounded, so the replay never holds one
write lock for its whole duration against the running scheduler (part AH)."""
    ...
    async def run(self, config: BacktestConfig) -> tuple[BacktestRun, ReplayStats]:
        ...
        async with session_scope(self._factory) as session:
            run = await BacktestRunRepository(session).create(
                _new_run(config, started_at=utc_now())
            )
            run_id = run.id
    ...
    async def _replay(self, config: BacktestConfig, *, run_id: int, stats: ReplayStats) -> None:
        ...
        for batch in _chunks(list(config.symbols), DEFAULT_SYMBOL_CHUNK):
            async with session_scope(self._factory) as session:
                for symbol in batch:
                    try:
                        count = await self._replay_symbol(
                            session=session,
                            symbol=symbol,
                            grid=grid,
                            config=config,
...
def _new_run(config: BacktestConfig, *, started_at: datetime) -> BacktestRun:
    return BacktestRun(
